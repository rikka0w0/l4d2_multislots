...
public OnPluginStart()
{
	...
	// Register cvars
	hMaxSurvivors = CreateConVar("l4d2_multislots_max_survivors", "8", "Maximum amount of survivors allowed", CVAR_FLAGS, true, 4.0, true, 8.0)
	hKickIdlers	= CreateConVar("l4d2_multislots_kickafk", "2", "Kick idle players? (0 = no  1 = player 10 min, admins kickimmune  2 = player 10 min, admins 10 min)", CVAR_FLAGS, true, 0.0, true, 2.0)
...
public Action:AddBot(Client, args)
{
	...
	{	
		PrintToChatAll("Survivor bot was spawned and teleported")
	}	
...
public Action:JoinTeam(client, args)
{
	...
	if(IsClientInGame(client))
	{
		...
		{	
			...
			{
				PrintHintText(client, "You are already a badass zombie hunter fool")
			}
			else if((DispatchKeyValue(client, "classname", "info_survivor_position") == true) && !IsAlive(client))
			{
				PrintHintText(client, "DIE DIE DIE is all you seem to do Gomer Pyle!")
			}
		}
		else if(IsClientIdle(client))
		{
			PrintHintText(client, "Press your mouse to hunt again as a survivor")
		}
		else
		{			
			if(TotalFreeBots() == 0)
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			if((GetClientTeam(i) == TEAM_SPECTATORS) && !IsFakeClient(i))
			{
				if(!IsClientIdle(i))
				{
					...
					PrintToChat(i, "\x01[\x04BAZINGA\x01] %s, type \x03!join\x01 to hunt down and kill ALL zombos!", PlayerName)
				}
				switch(GetConVarInt(hKickIdlers))
				{
					...
					case 1:
					{
						if(GetUserFlagBits(i) == 0)
						{
							giIdleTicks[i]++
							if(giIdleTicks[i] == 40)
								KickClient(i, "Hey gay cabellero, you were idle for 10 plus mins")
						}
					}
					case 2:
					{
						...
						{
							if(giIdleTicks[i] == 40)
								KickClient(i, "Master Bater, you were idle for 10 plus mins")
						}
						else
						{
							if(giIdleTicks[i] == 80)
								KickClient(i, "Cold, Gunny, and Huck are never idle for 20 mins")
						}
					}
				}
			}
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if(IsClientConnected(i) && IsClientInGame(i))		
		{
			if((GetClientTeam(i) == TEAM_SURVIVORS) && !IsFakeClient(i) && !IsAlive(i))
			{
				new String:PlayerName[100]
				GetClientName(i, PlayerName, sizeof(PlayerName))
				PrintToChat(i, "\x01[\x04BAZINGA!\x01] %s, Sheldon Cooper slays zombies better than you princess!", PlayerName)
			}
		}
	}	
	return Plugin_Continue
...
stock TakeOverBot(client, bool:completely)
{
	...
	if (bot==0)
	{
		PrintHintText(client, "Stay frosty, no bots are available")
		return
	}
